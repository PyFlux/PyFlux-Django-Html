...
def sideBar(user_id):
    if (user_id):
        ...
        sub_menu = AclPermissions.objects.filter(role_id__in=user_roles, parent_menu__in=main_parent_menu, view=1,
                                                 level=1).order_by('ordering')

        for sub_key in sub_menu:
            sub_parent_menu.append(sub_key.id)

        sub_sub_menu = AclPermissions.objects.filter(role_id__in=user_roles, parent_menu__in=sub_parent_menu, view=1,
                                                     level=2).order_by('ordering')
